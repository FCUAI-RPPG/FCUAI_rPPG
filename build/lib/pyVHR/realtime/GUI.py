...
def GUI_MENU():
    ...
    def create_run_window():
        fps = Params.fps_fixed if Params.fps_fixed is not None else get_fps(Params.videoFileName)
    ...
    while True:
        ...
        if event == '-APPLY-':
            ...
            if((values['-useCHROM-'])):
                ...
                Params.method = {'method_func': cupy_CHROM,'device_type': 'cuda', 'params': {}}
            elif((values['-useLGI-'])):
                Params.methodName = "LGI"
                Params.method = {'method_func': cpu_LGI,'device_type': 'cpu', 'params': {}}
